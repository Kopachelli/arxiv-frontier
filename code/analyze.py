...
def write_macros(cand, l1, fin, inc):
    ...
    if coded_path.exists():
        ...
        # claim-strength cross-tabulation (Table: verification by claim strength)
        strong_eval = {"HUMAN_EXPERT", "HELD_OUT_TRANSFER", "REAL_WORLD"}
        art_rows = {}
        if art_path.exists():
            art_rows = {r["arxiv_id"]: r for r in load("artifact-verification.csv")}

        def p(x, tot):
            return f"{x / tot:.0%}".replace("%", "\\%") if tot else "--"
        ...
        for cl, tag in [("DISCOVERY", "Disc"), ("CAPABILITY", "Cap"),
            ...
            if not sub:
                continue
            t = len(sub)
            m[f"pct{tag}StrongEval"] = p(sum(1 for r in sub if set(r["D5"].split(";")) & strong_eval), t)
            m[f"pct{tag}RealWorld"] = p(sum(1 for r in sub if "REAL_WORLD" in r["D5"].split(";")), t)
            m[f"pct{tag}HeldOut"] = p(sum(1 for r in sub if "HELD_OUT_TRANSFER" in r["D5"].split(";")), t)
            m[f"pct{tag}Audit"] = p(sum(1 for r in sub if r["D6"] != "NONE"), t)
            m[f"pct{tag}NoAudit"] = p(sum(1 for r in sub if r["D6"] == "NONE"), t)
        ...
        if art_path.exists():
            ...
            if ok:
                ...
                m["nDiscChecked"] = sum(1 for r in ok if r["D8"] == "DISCOVERY")
