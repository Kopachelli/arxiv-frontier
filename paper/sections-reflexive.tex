...
\nErrLogged{} failures are logged in \texttt{process-log/errors.md}; the \nErrReview{} that
arose during the review itself are taxonomised below. We report them with rates because rates
are what the literature we mapped almost never provides.
...
First, \textbf{every failure we caught ourselves was caught by a mechanical check that cost
almost nothing}: expected-versus-actual reconciliation, refusing to process empty input, and
running two raters against each other. None required sophisticated oversight. None would have been
...
for trust: the logs are timestamped in git history, the released data files permit independent
recomputation of the corpus statistics, and the failure modes named here are mechanically
detectable in the released artifacts. One study is an exception we record rather than gloss:
the double-coding reliability figures in \Section~3.5 cannot be recomputed, because the second-pass
codes were not preserved (Section~\ref{sec:corrections}). That is weaker than external audit, and we
prefer to say so.
