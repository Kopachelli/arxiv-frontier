...
The answer is lopsided in a specific and initially surprising way. The field grew roughly
\growthFactor{}-fold in nine quarters (\firstQn{} papers in \firstQ{} to \peakQn{} in
\peakQ{}), and by conventional standards it evaluates itself diligently: \pctBenchMetric{} of
the corpus reports benchmark metrics. But only \pctHeldOut{} of papers re-test a selected
result outside the loop that produced it, \pctAuditNone{} provide no auditability mechanism
of any kind --- no inspectable traces, no claim-to-evidence provenance, no reproduction
artifacts, no calibration --- and \pctHumanUnspec{} never state what the humans did.

Conditioning on claim strength separates two things that the aggregates blur. Among the
\nDiscovery{} papers claiming the AI produced new knowledge about the world, external
validation is dramatically \emph{better} than average --- \pctDiscRealWorld{} versus
\pctRealWorld{} corpus-wide, an interval (\ciDiscRealWorld{}) nowhere near the corpus rate.
Authors who claim a discovery do work to show the finding is real. Auditability is
\emph{no better}: \pctDiscAudit{} provide a mechanism against \pctAnyAudit{} overall, and at
$n=\nDiscPapers{}$ the interval (\ciDiscAudit{}) contains the corpus rate, so we do not claim
a difference. The asymmetry is the finding, and it is sharper than a difference would be:
authors claiming a discovery feel the burden of proving the finding and discharge it, and
feel no corresponding burden to expose how it was reached. A reader can often check that a
reported finding holds, while having no way to establish how the system reached it or what
