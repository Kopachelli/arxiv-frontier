...
computed over the \nArtChecked{} papers that both parsed and remain in the version~3 corpus:
the parse succeeded for \nArtParsed{} papers, and \nArtDropped{} of those were removed from
the corpus by amendment and so are not counted here.
...
\nRelSubstantial{} of the \nRelDims{} dimensions reach substantial agreement --- paper type
$\kappa=\kappaDOne$, claim strength $\kappaDEight$, code release $\kappaDSevenCode$, data
release $\kappaDSevenData$, domain $\kappaDFour$, autonomy $\kappaDTwo$, and (as multi-select
Jaccard) evaluation method $\kappaDFive$ and auditability mechanisms $\kappaDSix$. Two require
comment.
...
once are not measurements. Per-dimension results are in
\texttt{data/coding-reliability.csv} and are summarised in
\texttt{data/CODING-RELIABILITY-RESULTS.md}. Two limits of that release are stated in
Section~\ref{sec:corrections}: the confusion matrix is described in prose rather than tabulated, and
the second-pass codes this study was computed from were not preserved, so its $\kappa$ values
