...
validation is markedly \emph{better} than average (\pctDiscRealWorld{} versus
\pctRealWorld{} corpus-wide): authors who claim a discovery generally do work to show the
finding is real. Auditability is markedly \emph{worse} (\pctDiscNoAudit{} provide none,
versus \pctAuditNone{} overall). The field validates its outputs better than it exposes its
processes, and the gap is widest exactly where its claims are strongest. A reader can often
check that a reported finding holds, while having no way to establish how the system reached
it or what was discarded along the way.
